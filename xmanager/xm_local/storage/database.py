...
class GenericSqlConnector(SqlConnector):
  ...
  def create_engine(settings: SqlConnectionSettings) -> Engine:
    ...
    url = getattr(sqlalchemy, 'URL', sqlalchemy.engine.url.URL).create(
...
class CloudSqlConnector(SqlConnector):
  ...
  def create_engine(settings: SqlConnectionSettings) -> Engine:
    ...
    url = getattr(sqlalchemy, 'URL', sqlalchemy.engine.url.URL).create(
        drivername=f'{settings.backend}+{driver}', host='localhost'
    )
...
class Database:
  ...
  def maybe_migrate_database_version(self):
    ...
    with self.engine.connect() as connection:
      legacy_sqlite_db = sqlalchemy.inspect(connection).has_table(
          'VersionHistory'
      )
